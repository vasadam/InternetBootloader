...
* Vas Adam             12/04/13  Modified for DSN-PC: added #define ERASE_LAST_PAGE
*                      12/12/13  Added XTEADecrypt()
************************************************************************************/
#include <p18cxxx.h>
#include <string.h>
...
/* Starting address of the 2-byte DSN-PC ID (0x1FFF6-0x1FFF7) */
#define DSNPC_ID_ADDR           (0x1FFF6)
...
#define MAC_ADDRESS5		(*(far rom const BYTE*)(DSNPC_ID_ADDR+1))
#define MAC_ADDRESS6		(*(far rom const BYTE*)DSNPC_ID_ADDR)
#define IP_ADDRESS0		192
#define IP_ADDRESS1		168
#define IP_ADDRESS2		0
#define IP_ADDRESS3		(110ul+*(far rom const BYTE*)DSNPC_ID_ADDR)
...
static void XTEADecrypt(DWORD *data, WORD len);
...
// buffer for restoring last page immediately after erase
...
#define XTEA_NUM_ITERATIONS 32u

// XTEA variables
#pragma udata XTEA_udata
static WORD  decrypt_i;
static DWORD v0;
static DWORD v1;
static DWORD sum;
static DWORD cipher_key[4];
#pragma udata
	...
	switch(wvTFTPOpcode.Val)
	{
		...
		case TFTP_DATA:		// Writing data
			...
			wNeededBlock++;
                        ...
                        // Decrypt the data block we received
                        // Its length will always be a multiple of 8 bytes since we're using block encryption
                        XTEADecrypt((DWORD*)AppDataPtr, AppDataLen);

                        // Strip off any padding if necessary
                        while(*(AppDataPtr+AppDataLen-1) == 0x00u)
                            AppDataLen--;
...
/******************************************************************************
* Function:        void XTEADecrypt(unsigned int *data, unsigned short int len)
*
* PreCondition:    None
*
* Input:           binary data of 4-byte blocks, length in bytes
*
* Output:          None
*
* Side Effects:    None
*
* Overview:        Decrypts a 4-byte binary data block encrypted with XTEA
*
* Note:            None
*****************************************************************************/
void XTEADecrypt(DWORD *data, WORD len)
{
    decrypt_i = 0;
    cipher_key[0] = 0x11111111;
    cipher_key[1] = 0x22222222;
    cipher_key[2] = 0x33333333;
    cipher_key[3] = 0x44444444;

    while(decrypt_i < len)
    {
        sum = DELTA * XTEA_NUM_ITERATIONS;
        v0 = *data;
        v1 = *(data+1);

        while(sum != 0u)
        {
            v1 -= (((v0 << 4) ^ (v0 >> 5)) + v0) ^ (sum + cipher_key[(sum>>11)&3]);
            sum -= DELTA;
            v0 -= (((v1 << 4) ^ (v1 >> 5)) + v1) ^ (sum + cipher_key[sum&3]);
        }
...
static BOOL WriteFlashBlock(DWORD Address, BYTE *BlockData, BYTE *StatusData)
{
	...
	{
            // Configuration bits occupy 8 bytes, but the minimum
            // flash memory write size is 64 bytes. DSN-PC ID and
            // sensor calibration data are located just before the
            // configuration bits, and we have to erase the whole
            // last page (minimum erase size is 1024 bytes), even if
            // we only want to modify config bits and leave DSN-PC
            // data untouched. Therefore, copy these data
            // from flash memory to BlockData[] to write it back
            // after erasing the last page.
            TBLPTR = INTERNAL_FLASH_SIZE - FLASH_WRITE_SIZE;
            for (w = 0; w < FLASH_WRITE_SIZE - CONFIG_FUSE_SIZE; w++)
            {
                _asm TBLRDPOSTINC _endasm
                BlockData[w] = TABLAT;
            }
            ...
            // See if the current config fuses differ from what we want
            ...
            TBLPTR = INTERNAL_FLASH_SIZE - CONFIG_FUSE_SIZE;
            for(w = FLASH_WRITE_SIZE - CONFIG_FUSE_SIZE; w < FLASH_WRITE_SIZE; w++)
            {
                _asm TBLRDPOSTINC _endasm
                i = BlockData[w];
                if(TABLAT != i)
...
static void MACFlush(void)
{
	...
	// If you don't wait long enough, the TX logic won't be finished resetting.
        ...
        // kikommentelte: vas_adam 2013-02-22
        // Oka: static vĂ�Â„Ă‚Â‚Ă�Â‹Ă‚Â‡ltozĂ�Â„Ă‚Â‚Ă�Â…Ă‚Â‚k nem kapnak kezdĂ�Â„Ă‚ÂąĂ�Â˘Ă‚Â€Ă‚Â�Ă�Â„Ă‚Â‚Ă�Â‚Ă‚Â©rtĂ�Â„Ă‚Â‚Ă�Â‚Ă‚Â©ket a linker script beĂ�Â„Ă‚Â‚Ă�Â‹Ă‚Â‡llĂ�Â„Ă‚Â‚Ă�Â‚Ă‚Â­tĂ�Â„Ă‚Â‚Ă�Â‹Ă‚Â‡sa miatt,
        // ld. "VERY IMPORTANT NOTE" ennek a fĂ�Â„Ă‚Â‚Ă�Â‹Ă‚Â‡jlnak az elejĂ�Â„Ă‚Â‚Ă�Â‚Ă‚Â©n
        //{static volatile BYTE i = 8; while(i--);}
        {static volatile BYTE i; i = 8; while(i--);}
	EIRbits.TXERIF = 0;
